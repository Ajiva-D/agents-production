...
export const summarizeMessages = async (messages: AIMessage[]) => {
  const response = await runLLM({
    messages,
    systemPrompt: `Your job is to summarizw the given messages to be used in another LLM's system prompt. Summarize it play by play`,
    temperature: 0.3,
  })
